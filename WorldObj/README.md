...
반복 조회를 공통 코드에서 처리하기 위해 `WorldObj` 컴포넌트 캐시를 구성했습니다. `WorldObj`는 생성 시점에 부착된 컴포넌트와 사용자 정의 부모 타입을 저장하고, 이후 `Has<T>()`, `Get<T>()`, `TryGet<T>()`로 조회합니다.
...
- 사용자 정의 부모 타입도 같은 컴포넌트에 연결합니다.

호출부는 구체 타입을 미리 고정하지 않고 캐시에 등록된 기능 타입을 확인할 수 있습니다.

## 확인한 내용

`Miner : Production : Structure` 상속 관계에서 구체 타입과 부모 타입이 같은 컴포넌트를 반환하는지 확인했습니다. 등록되지 않은 타입은 `Has<T>()`와 `TryGet<T>()`에서 `false`, `Get<T>()`에서 `null`을 반환했습니다.
