...
몬스터는 가장 가까운 대상만 선택하지 않고 주변 대상의 어그로와 거리, 현재 해당 건물을 공격 중인 몬스터 수, 기존 타깃 여부를 함께 점수로 계산합니다.
...
피해량과 공격 속도에 따라 추가 어그로가 누적되며 최대값을 제한했습니다. 추가 어그로는 일정 시간마다 감소하여 전투 상황이 바뀌면 몬스터의 관심도도 함께 변화합니다.

공격 가능한 유닛·타워·플레이어·핵심 건물은 기본 거리 가중치를 사용하고, 직접 공격하기 어려운 기타 구조물은 더 큰 거리 가중치를 적용했습니다. 또한 이미 여러 몬스터가 선택한 건물의 점수를 낮춰 특정 목표 하나에 과도하게 집중되는 상황을 줄였습니다.
...
타깃 점수와 현재 공격 중인 몬스터 수를 함께 반영하여 여러 몬스터가 하나의 건물에만 집중되는 상황을 줄이고, 전투 상황에 따라 우선순위가 자연스럽게 바뀌도록 구성했습니다.
