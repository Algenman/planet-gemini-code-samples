...
`UnitDrag`는 마우스 입력을 해석하여 단일 선택, 영역 선택, 화면 내 동일 종류 선택을 처리합니다. 선택 결과와 이동·공격 이동·패트롤·정지·지정 대상 공격 명령은 이벤트로 전달합니다.
...
선택한 유닛을 `UnitGroupCtrl`에서 하나의 그룹으로 관리하고 이동, 공격 이동, 패트롤, 정지, 지정 대상 공격 명령을 각 유닛에 전달합니다. Client 입력은 ServerRpc로 전달하고, `UnitAi`와 `UnitCommonAi`가 서버에서 명령 상태와 행동 분기를 갱신합니다.
...
- `Idle`: 타깃이 없고 스포너 범위 안에 있으면 대기
- `Patrol`: 스폰 지점 주변 순찰
- `Trace`: 발견한 타깃 추적
- `Attack`: 공격 범위에 진입한 타깃 공격
- `ReturnToSpawn`: 타깃 소실 후 스포너 범위 밖에 있거나 활동 범위를 벗어나면 복귀
...
몬스터는 주변 후보를 공격 가능 대상으로 분류된 후보와 그 외 후보로 나누고, 서로 다른 거리 가중치를 적용합니다.
...
후보 점수는 일정 주기로 다시 계산합니다. 처음 선택한 대상이 이동하거나 멀어지면 변경된 거리를 반영하고, 기존 타깃 `+3`은 작은 점수 변화로 대상이 자주 바뀌는 것을 줄입니다. 타깃 변경·소실과 몬스터 사망 시 이전 건물의 `monsterTargetAmount`를 감소시킵니다.
...
UnitCommonAi
 ├─ UnitAi: 이동 / 패트롤 / 공격 이동 / 정지 / 지정 대상 공격
...
- 이동, 공격 이동, 패트롤, 정지, 지정 대상 공격 입력을 이벤트로 분리
...
- 어그로, 거리, 공격 집중도, 기존 타깃을 반영한 타깃 점수 계산
