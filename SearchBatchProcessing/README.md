...
유닛과 타워는 각각 다른 탐색 주기를 사용합니다. 한 프레임에는 `searchCap`으로 정한 수만큼만 탐색하고, 남은 대상은 다음 프레임에서 이어서 처리합니다.

또한 탐색 루프 중 리스트가 변경되지 않도록 제거 요청은 대기 목록에 모아두었다가 탐색 처리 밖에서 정리했습니다.
...
- 제거 요청은 즉시 삭제하지 않고 대기 목록에 모아 처리
...
Unity Profiler에서 주변 탐색 작업이 `searchCap`을 기준으로 여러 프레임에 나뉘어 실행되는 것을 확인했습니다. 당시 측정값은 기록하지 않아 향상률은 적지 않았습니다.

탐색 중 유닛과 타워를 제거하는 상황도 확인했으며, 대기 목록을 정리한 뒤 다음 탐색이 컬렉션 변경 오류 없이 이어졌습니다.
